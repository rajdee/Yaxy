...
function Yaxy(port, httpsPort, certs) {
    if (!(this instanceof Yaxy)) {
        return new Yaxy(port, httpsPort, certs);
    }
    ...
    this._server.on('upgrade', this._onRequest.bind(this, 'ws:'));
    ...
    if (httpsPort) {
        ...
        var options = {
            key: read(resolve(certs.key || (__dirname + '/../ssl/privatekey.pem'))),
        ...
        this._httpsServer.on('upgrade', this._onRequest.bind(this, 'wss:'));
...
Yaxy.prototype = {
    ...
    setAuth: function(userPasswd) {
        this._auth = 'Basic ' + new Buffer(userPasswd).toString('base64');
    },


    _onRequest: function(protocol, req, res) {
        if (this._auth && req.headers['proxy-authorization'] !== this._auth) {
    ...
    _onConnect: function(request, socketRequest, head) {
        if (this._auth && request.headers['proxy-authorization'] !== this._auth) {
            socketRequest.write("HTTP/" + request.httpVersion + " 407 Proxy Authentication Required\r\nProxy-Authenticate: Basic\r\n\r\n");
            socketRequest.end();
            return;
        }
        ...
        if (ph.port === '80') {
            port = this._httpPort;
        } else if (!port || this._sslHosts.indexOf(request.url) === -1) {
            host = ph.hostname;
            port = Number(ph.port);
        }
...
function State(protocol, req, res, head) {
    this._protocol = protocol;
    this._req = req;
    this._res = res;
    this._requestHead = head;
    this._proxy = null;
    ...
    if (!/^(http|ws)s?:\/\//.test(this._req.url)) {
...
State.prototype = {
    ...
    doRequest: function() {
        var secure = this._protocol === 'https:' || this._protocol === 'wss:';
        ...
        if (this._proxy && this._proxy.user) {
            ...
            delete this._requestHeaders['proxy-authorization'];
        }
        ...
        var hostByProxy = this._proxy ? this._proxy.host : host;
        var portByProxy = this._proxy ? this._proxy.port : port;
        if (this._protocol === 'http:' || this._protocol === 'https:') {
            var requestOptions = {
                host: hostByProxy,
                port: portByProxy,
                method: this._req.method,
                path: this._proxy ? this._protocol + '//' + host + path : path,
                headers: headers,
                rejectUnauthorized: false
            };

            var clientReq = require(secure ? 'https' : 'http').request(requestOptions, (clientRes) => {
                var statusCode = this._responseStatus || clientRes.statusCode;
                var headers = this._buildResponseHeaders(clientRes.headers);
                this._res.writeHead(statusCode, headers);
                clientRes.pipe(this._res);
            });
            ...
            this.ready(function() {
                this._requestBodyChunks.forEach((chunk) => {
            ...
            if (!this._req.connection.__yaxy_clientReqs) {
                ...
                this._req.connection.once('close', () => {
        ...
        } else if (this._protocol === 'ws:' || this._protocol === 'wss:') {
            var websocket = require('websocket-driver');
            if (websocket.isWebSocket(this._req)) {
                var toclient = websocket.http(this._req);
                toclient.io.write(this._requestHead);
                this._res.pipe(toclient.io).pipe(this._res);

                var toserver = websocket.client(this._url);
                toserver.addExtension(deflate);
                
                Object.keys(headers).forEach((header) => {
                    toserver.setHeader(header, headers[header]);
                });
                toclient.messages.pipe(toserver.messages).pipe(toclient.messages);

                var tcp = require('net').connect(portByProxy, hostByProxy, () => {
                    tcp.pipe(toserver.io).pipe(tcp);
                    toclient.start();
                    toserver.start();
                });
            }
    ...
    _matchers: {
        method: function(method) {
            return method ? this._matchers._string(method, this._req.method) : true;
        },
